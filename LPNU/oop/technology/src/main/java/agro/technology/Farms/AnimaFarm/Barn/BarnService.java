...
public class BarnService {
    ...
    public boolean addAnimal(Barn barn, int amount) {
        ...
        if (budgetService.getBudget() - animalService.getAnimal(barn.getType()).getPrice() * amount >= 0
                && barn.getSize() - barn.animalsAmount >= amount) {
            budgetService.updateBudget(-animalService.getAnimal(barn.getType()).getPrice() * amount);
    ...
    public int canAddAnimals(Barn barn) {
        ...
        if (animalService.getAnimal(barn.getType()).getPrice() != 0 &&
                barn.getSize()
                        - barn.animalsAmount >= (int) (budget / animalService.getAnimal(barn.getType()).getPrice())) {

            return (int) (budget / animalService.getAnimal(barn.getType()).getPrice());
        }
